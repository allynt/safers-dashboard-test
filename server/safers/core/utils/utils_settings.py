...
class DynamicSetting(object):
    ...
    def __init__(self, source, default_value):
        ...
        if len(source.split(".")) != 3:
            raise ImproperlyConfigured(
                "Invalid DynamicSetting value; format is <app>.<model>.<attr>"
            )
    ...
    def value(self):
        ...
        try:
            ...
            assert hasattr(model, "_singleton_id")
            instance, created = model.objects.get_or_create(_singleton_id=model.DEFAULT_SINGLETON_ID)
            ...
            raise ImproperlyConfigured(
                "Invalid DynamicSetting; must use SingletonMixin"
            )
